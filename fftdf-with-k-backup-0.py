...
def get_coul(c, ke=None, kmesh=None, cisdf=10.0):
    ...
    for ao_ks_etc, p0, p1 in df_obj.aoR_loop(grids, vk):
        phi_k = numpy.asarray(ao_ks_etc[0])
        phi_k = phi_k * numpy.sqrt(weigh)[None, p0:p1, None]

        assert phi_k.shape == (nkpt, p1 - p0, nao)

        phi2_k = einsum("kgm,kIm->kgI", phi_k.conj(), x_k)
        assert phi2_k.shape == (nkpt, p1 - p0, nip)

        phi2_s = einsum("Rk,kgI->RgI", phase, phi2_k)
        assert abs(phi2_s.imag).max() < 1e-10

        y_s = phi2_s * phi2_s
        y_k = einsum("RgI,Rk->kgI", y_s, phase)
        assert y_k.shape == (nkpt, p1 - p0, nip)

        z_k = [numpy.dot(y, x4inv) for y, x4inv in zip(y_k, x4inv_k)]
    ...
    for k1, vk1 in enumerate(vk):
        coulg_k = pbctools.get_coulG(c, k=vk1, mesh=mesh, Gv=gv)
        assert coulg_k.shape == (ng, )

        def func(i0, i1):
            zk = fswp["zk"][k1, :, i0:i1]
            assert zk.shape == (numpy.prod(mesh), i1 - i0)
            return einsum("gI,gJ,g->IJ", zk, zk, coulg_k)

        from pyscf.lib import map_with_prefetch
        map_with_prefetch(func, prange(0, nip, blkszie))
...
if __name__ == "__main__":
    ...
    cell.verbose = 5
    get_coul(cell, ke=200, kmesh=[2, 2, 2], cisdf=30.0)
