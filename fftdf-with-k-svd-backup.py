...
def get_coul(df_obj, k0=10.0, kmesh=None, cisdf=0.6, verbose=5, blksize=16000):
    ...
    for q, x4q in enumerate(x4_k):
        u, s, vh = scipy.linalg.svd(x4q, full_matrices=False)
        err = abs(x4q - u @ s[:, None] @ vh).max()
        assert err < 1e-10

        rank = max(rank, numpy.count_nonzero(s > 1e-10))
        u_k.append(u)
        s_k.append(s)

    u_k = numpy.asarray(u_k)[:, :, :rank]
    s_k = numpy.asarray(s_k)[:, :rank]
    assert u_k.shape == (nkpt, nip, rank)
    assert s_k.shape == (rank, )

    x_k = einsum("kIm,kIJ->kJm", x_k, u_k)
    assert x_k.shape == (nkpt, rank, nao)
    nip = rank
    x4inv_k = 1.0 / s_k
